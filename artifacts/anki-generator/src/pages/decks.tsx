...
function getAllDescendants(
  deckId: number,
  childrenMap: Map<number, DeckWithParent[]>,
): DeckWithParent[] {
  const direct = childrenMap.get(deckId) ?? [];
  return [
    ...direct,
    ...direct.flatMap((d) => getAllDescendants(d.id, childrenMap)),
  ];
}
...
function DeckRow({
  ...
  const children = (deckChildrenMap.get(deck.id) ?? []).sort((a, b) =>
  ...
  const totalCards =
    deck.cardCount + allDescendants.reduce((s, d) => s + d.cardCount, 0);
  ...
  const iconColor =
    clampedDepth === 0
      ...
      : clampedDepth === 1
        ...
        : "text-violet-500";
  const iconBoxSize =
    clampedDepth === 0 ? "h-9 w-9" : clampedDepth === 1 ? "h-7 w-7" : "h-6 w-6";
  const iconSize =
    clampedDepth === 0
      ? "h-4 w-4"
      : clampedDepth === 1
      ...
      <div className="relative group">
        {selectMode && (
          ...
          >
            <Checkbox
              checked={isSelected}
              className={`${checkboxClass} bg-background border-2 shadow-sm`}
            />
          </div>
        )}
        <Link href={selectMode ? "#" : `/decks/${deck.id}`}>
          <Card
            ...
            onClick={
              selectMode
                ? (e) => toggleSelect(deck.id, e as React.MouseEvent)
                : undefined
            }
          >
            <CardContent className={cardPadding}>
              <div className="flex items-center gap-2.5">
                ...
                <div
                  className={`${iconBoxSize} rounded-md flex items-center justify-center shrink-0 ${iconBg}`}
                >
                  {hasChildren ? (
                ...
                <div className={`flex-1 min-w-0 ${selectMode ? "pl-5" : ""}`}>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {hasChildren && (
                      <Badge
                        variant="outline"
                        className="text-xs shrink-0 py-0 px-1.5"
                      >
                        {children.length} sub-deck
                        {children.length !== 1 ? "s" : ""}
                      </Badge>
                    )}
                  </div>
        ...
        <div className={indentClass}>
          ...
          {!selectMode && (
            <button
              ...
              onClick={() =>
                openDeckForm({ type: "new-subdeck", parentId: deck.id })
              }
            >
              <Plus className="h-3 w-3" />
              Add sub-deck to{" "}
              <span className="font-medium ml-0.5">{deck.name}</span>
            </button>
          )}
        </div>
      )}
...
export default function Decks() {
  ...
  useEffect(() => {
    ...
    if (wantsNew || combined || title) {
      ...
      ["new", "shared_text", "shared_url", "shared_title"].forEach((k) =>
  ...
  const [deckFormMode, setDeckFormMode] = useState<DeckFormMode>({
    type: "new-topic",
  });
  ...
  const handleExportLibraryApkg = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Downloaded ${exportName}.apkg — import into Anki to add every deck.`,
      });
    } catch (err) {
      toast({
        title: "Export failed",
  ...
  const handleExportAllJson = async () => {
    ...
    try {
      ...
      toast({
        title: "Library exported",
        description: "All main topics saved to a single JSON file.",
      });
    } catch (err) {
      toast({
        title: "Export failed",
  ...
  const totalCards =
    (decks as DeckWithParent[] | undefined)?.reduce(
      (sum, d) => sum + d.cardCount,
      0,
    ) ?? 0;

  const { rootDecks, rootFlashcardDecks, rootQbankDecks, deckChildrenMap } =
    useMemo(() => {
      const all = (decks as DeckWithParent[] | undefined) ?? [];
      const root = all
        .filter((d) => !d.parentId)
      ...
      return {
        rootDecks: root,
        rootFlashcardDecks: flashcards,
        rootQbankDecks: qbanks,
        deckChildrenMap: byParent,
      };
    }, [decks]);
  ...
  const filterBySearch = (list: DeckWithParent[]) => {
    ...
    function matchesSearch(d: DeckWithParent): boolean {
      if (
        ...
        return true;
      return (deckChildrenMap.get(d.id) ?? []).some((child) =>
        matchesSearch(child),
      );
    }
  ...
  const filteredFlashcards = useMemo(
    () => filterBySearch(rootFlashcardDecks),
    [rootFlashcardDecks, deckChildrenMap, search],
  );
  const filteredQbanks = useMemo(
    () => filterBySearch(rootQbankDecks),
    [rootQbankDecks, deckChildrenMap, search],
  );
  ...
  const openDeckForm = (mode: DeckFormMode) => {
    setDeckFormMode(mode);
  ...
  const handleDelete = (id: number, e: React.MouseEvent) => {
    ...
    const totalCards =
      (target?.cardCount ?? 0) +
      descendants.reduce((s, d) => s + d.cardCount, 0);
    const msg =
  ...
  const openMergeDialog = () => {
    ...
    const selectedDecks =
      (decks as DeckWithParent[] | undefined)?.filter((d) =>
  ...
  const handleMerge = async () => {
    ...
    if (!name) {
      toast({
        title: "Name required",
        description: "Give the merged deck a name.",
        variant: "destructive",
      });
      return;
    }
      ...
      toast({
        ...
        description: `“${data.name}” created with ${data.cardCount} card${data.cardCount === 1 ? "" : "s"}${
          mergeDeleteOriginals
            ? `. ${selectedIds.size} original deck${selectedIds.size === 1 ? "" : "s"} removed.`
            : "."
        }`,
      });
      setMergeOpen(false);
  ...
  const handleExportApkg = async () => {
    ...
    try {
      ...
      if (!resp.ok)
        throw new Error(
          (await resp.json().catch(() => ({}))).error ?? "Export failed.",
        );
      const blob = await resp.blob();
      const url = URL.createObjectURL(blob);
      const a = Object.assign(document.createElement("a"), {
      ...
      toast({
        ...
        description: `Downloaded ${exportName}.apkg (all nested decks included).`,
      });
    } catch (err) {
      toast({
        title: "Export failed",
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-32">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-3xl font-serif font-bold text-primary tracking-tight leading-none">
              Library
            </h1>
            ...
            <>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-60">
                  ...
                  <DropdownMenuItem
                    className="gap-2.5 cursor-pointer"
                    onClick={() => openDeckForm({ type: "new-subdeck" })}
                  >
      ...
      {allDecksCount > 0 && (
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-xl border border-border/60 bg-card/60 backdrop-blur-sm p-3.5 shadow-sm">
            ...
            <div className="mt-1.5 text-2xl font-serif font-bold text-foreground">
              {topicsCount}
            </div>
          </div>
          <div className="rounded-xl border border-border/60 bg-card/60 backdrop-blur-sm p-3.5 shadow-sm">
      ...
      {allDecksCount > 0 && (
        <div className="relative">
          ...
          {search && (
            <button
              onClick={() => setSearch("")}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground p-1 rounded hover:bg-muted transition-colors"
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 px-6 border-2 border-dashed border-border/60 rounded-2xl bg-gradient-to-b from-card/60 to-muted/20">
          ...
          <h3 className="text-xl font-serif font-semibold mb-1.5">
            Your library is empty
          </h3>
          <p className="text-muted-foreground text-sm mb-6 max-w-sm mx-auto">
            Start by creating a main topic, generating AI flashcards, or
            building a question bank from your study material.
          </p>
        ...
        <Tabs
          value={libraryTab}
          onValueChange={(v) => setLibraryTab(v as "decks" | "qbanks")}
          className="w-full"
        >
          <TabsList className="w-full sm:w-auto h-10 p-1 bg-muted/60">
            <TabsTrigger
              value="decks"
              className="flex-1 sm:flex-none gap-1.5 h-8 px-4 data-[state=active]:shadow-sm"
            >
              <BookOpen className="h-3.5 w-3.5" /> Decks
              <span className="ml-1 text-xs text-muted-foreground">
                {rootFlashcardDecks.length}
              </span>
            </TabsTrigger>
            <TabsTrigger
              value="qbanks"
            ...
            {rootFlashcardDecks.length === 0 ? (
              <div className="text-center py-16 px-6 border-2 border-dashed border-border/60 rounded-2xl bg-card/60">
                ...
                <p className="text-sm text-muted-foreground mt-1 mb-4">
                  Generate a deck of flashcards from your notes or PDFs.
                </p>
                <Button
                  className="gap-2"
                  onClick={() => openGenerateSheet("deck")}
          ...
          <TabsContent value="qbanks" className="mt-4">
            {rootQbankDecks.length === 0 ? (
              <div className="text-center py-16 px-6 border-2 border-dashed border-border/60 rounded-2xl bg-card/60">
                ...
                <p className="text-sm text-muted-foreground mt-1 mb-4">
                  Generate a UWorld-style MCQ question bank from your study
                  material.
                </p>
                <Button
                  className="gap-2"
              ...
              <div className="text-center py-16 px-6 border-2 border-dashed border-border/60 rounded-2xl bg-card/60">
                <p className="font-medium">
                  No question banks match "{search}"
                </p>
                <Button
                  variant="ghost"
                  className="mt-3"
      ...
      <GenerateSheet
        ...
        onDone={() => {
          setLibraryTab(generateMode === "qbank" ? "qbanks" : "decks");
        }}
      ...
      {selectMode && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-4 duration-200">
          <div className="flex items-center gap-3 bg-card/95 backdrop-blur-md border border-border shadow-2xl rounded-2xl px-4 py-2.5 pl-5">
            <div className="flex items-center gap-2">
              <div
                className={`h-7 w-7 rounded-full flex items-center justify-center ${selectedIds.size > 0 ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}
              >
            ...
            <Button
              ...
              title={
                selectedIds.size < 2
                  ? "Select at least 2 decks to merge"
      ...
      <Dialog open={mergeOpen} onOpenChange={setMergeOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Combines every card from the selected decks (and their sub-decks)
              into one new deck.
            </DialogDescription>
          </DialogHeader>
            ...
            <label className="flex items-start gap-2.5 text-sm cursor-pointer rounded-lg border p-3 hover:bg-muted/30 transition-colors">
              ...
              <span>
                <span className="font-medium block">
                  Delete original decks after merging
                </span>
                <span className="text-xs text-muted-foreground">
                  Removes the source decks and any sub-decks they contain.
                  Cannot be undone.
                </span>
              </span>
            </label>
