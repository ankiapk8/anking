...
export function GenerateSheet({
  ...
  const [emptyParentId, setEmptyParentId] = useState<string>(
  ...
  const selectedEmptyParent = parentOptions.find(
    (o) => o.id.toString() === emptyParentId,
  );
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="w-full sm:max-w-lg overflow-y-auto p-0 bg-gradient-to-b from-background to-muted/20"
      >
        ...
        <div className="px-6 py-5">
          <Tabs defaultValue="generate" className="w-full">
            <TabsList className="w-full mb-6 h-10 p-1 bg-muted/60">
              <TabsTrigger
                value="generate"
                className="flex-1 gap-1.5 h-8 data-[state=active]:shadow-sm"
              >
                {isQbank ? (
            ...
            <TabsContent value="empty" className="space-y-4 mt-0">
              ...
              <div className="space-y-2">
                <Label htmlFor="emptyName">
                  {isQbank ? "Question Bank Name" : "Deck Name"}
                </Label>
                <Input
                  id="emptyName"
                  value={emptyName}
                  ...
                  placeholder={
                    isQbank
                      ? "e.g. Cardiology QBank"
                      : "e.g. Spanish Vocabulary"
                  }
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="emptyDesc">
                  Description{" "}
              ...
              <Button
                ...
                disabled={!emptyName.trim() || isCreating}
              >
                {isCreating ? (
                  <>
