...
function estimatedCards(
  ...
  const goal =
    typeof target === "number" && target > 0 ? target : DEFAULT_TARGET_CARDS;
...
export function GenerateForm({
  ...
  useEffect(() => {
    if (prefilledText || prefilledDeckName) {
      ...
      if (prefilledDeckName)
        setManualDeckName((prev) => prev || prefilledDeckName);
    }
  ...
  const hasManual =
    manualText.trim().length > 0 && manualDeckName.trim().length > 0;
  const canGenerate =
    !isExtracting && !isGeneratingAll && (readyFiles.length > 0 || hasManual);
    ...
    async (file: File) => {
      ...
      try {
        if (isTxt) {
          ...
          const { text, pageImages, pageTexts } = await extractPdf(
            buffer,
          ...
          updateFile(id, {
            ...
            pageTexts,
            progress: "",
            deckType: pageImages.length > 0 ? "both" : "text",
          });
        }
      } catch (error) {
        const message =
              ...
              >
                <CardContent className="p-3.5 space-y-2.5">
                  <div className="flex items-center gap-2.5">
                    ...
                    {f.status === "done" && (
                      <Badge className="text-[10px] shrink-0 bg-green-600 hover:bg-green-600 px-1.5 py-0 h-5">
                    ...
                    <div className="space-y-2">
                      <div className="space-y-1">
                        ...
                        <Input
                          value={f.deckName}
                          onChange={(e) =>
                            updateFile(f.id, { deckName: e.target.value })
                          }
                            ...
                            {[
                              ...
                              return (
                                <button
                                  ...
                                  disabled={isGeneratingAll}
                                  onClick={() =>
                                    updateFile(f.id, { deckType: opt.value })
                                  }
                        ...
                        if (target > capacity && capacity > 0) {
                          return (
                            <p className="text-[10px] text-emerald-600 dark:text-emerald-500">
                              Text content likely only supports ~{capacity}{" "}
                              cards — target will be capped.
                            </p>
                          );
                        }
                        return null;
        ...
        {manualText.trim().length > 0 && (
          ...
          >
            ...
            {(() => {
              ...
              const target =
                typeof manualCardCount === "number"
                  ? manualCardCount
                  : DEFAULT_TARGET_CARDS;
              if (target > capacity && capacity > 0) {
                return (
                  <p className="text-[10px] text-emerald-600 dark:text-emerald-500">
                    Content likely only supports ~{capacity} cards — target will
                    be capped.
                  </p>
                );
              }
