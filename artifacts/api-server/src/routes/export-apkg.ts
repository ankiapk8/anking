...
function buildAnkiFields(
  ...
  if (isMcq) {
    ...
    if (choices.length > 0) {
      const correct =
        typeof card.correctIndex === "number" ? card.correctIndex : -1;
      const choicesHtml = choices
        .map(
          (c, i) =>
            `<div style="margin:4px 0">${letterFor(i)}. ${toAnkiHtml(c)}</div>`,
        )
        .join("");
      frontHtml = `${frontHtml}<div style="margin-top:12px">${choicesHtml}</div>`;
      const correctLine =
        correct >= 0 && correct < choices.length
...
function addDeckEntry(
  ...
  sqlDb
    .prepare("UPDATE col SET decks=:d WHERE id=1")
    .getAsObject({ ":d": JSON.stringify(decks) });
}
...
function collectAllDescendantIds(
  ...
  const direct = allDecks.filter(
    (d) => d.parentId !== null && parentIds.includes(d.parentId!),
  );
...
router.post("/export-apkg", async (req, res, next): Promise<void> => {
  ...
  const autoDecks = allDecksInDb.filter(
    (d) => allDescendantIds.includes(d.id) && !ids.includes(d.id),
  );

  // De-duplicate
  const allDeckMap = new Map(
    [...requestedDecks, ...autoDecks].map((d) => [d.id, d]),
  );
  ...
  const exportRoots = allDecks.filter(
    (d) => !d.parentId || !allExportIds.has(d.parentId),
  );
  ...
  const collectionBuffer = Buffer.from(
    sqlExport.buffer,
    sqlExport.byteOffset,
    sqlExport.byteLength,
  );
